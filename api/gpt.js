...
class ClickbaitGenerator {
    constructor() {
        ...
        this.model = 'gpt-4o'; // Or use gpt-3.5-turbo for cheaper option
    }
...
Generate exactly ${count} different clickbait titles, numbered 1-${count}:
    ...
    async generateTitles(article, options = {}) {
        try {
            ...
            console.log('generating clickbait titles with OpenAI ... \n');

            const response = await this.openai.chat.completions.create({
                model: this.model,
                messages: [
                    { role: 'system', content: 'You are an expert clickbait headline generator.' },
